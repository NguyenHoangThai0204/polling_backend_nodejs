...
const contractAddress = "0x19CE7f29326871a3E3452A66BCd5792e43fD326B";
...
// check địa chỉ voter đã vote chưa
routers.get("/checkVoted", async (req, res) => {
  const { pollId, voter } = req.body;
  try {
    const result = await myContract.methods.hasVoted(pollId, voter).call();
    res.json({ message: "Check voted successfully", result });
  } catch (error) {
    console.log(error);
    res.status(500).json({ error: error.message });
  }
